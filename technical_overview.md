...
1. **Deep Research:** A lead agent delegates specific topics to parallel research sub-agents who write findings to a shared VFS.  
2. **Autonomous Coding:** A lead agent plans a feature, a coder sub-agent writes the code in a sandbox, and a reviewer sub-agent validates it.  
3. **Enterprise Data Analysis:** Orchestrating sub-agents across different siloed databases (SQL, Vector, ERP) without overflowing the main context window.
...
**A:** Yes, but it is a better practice to give sub-agents specific tools for their vertical. If a sub-agent needs access to the VFS, it must be explicitly provided with the read_file/write_file tools in its toolset.
